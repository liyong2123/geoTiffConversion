    ...
    for root, dirs, files in os.walk(os.getcwd()):
        ...
        for f in files:
            if f.endswith(".TIF"):
                ...
                meta = f
    if(arr == "" or fi == ""):
        print("No Valid Files Found in Folder or Invalid File")
        exit(0)
...
def askdir():
    try:
        root = Tk()
        root.withdraw()
        folder = filedialog.askdirectory(title='Choose directory for conversion')
        root.update()
        print(folder)
        if folder == None:
            raise Exception()
...
def main():
    ...
    arr, fi, meta, depth = findFile(askdir())
    f: str = fi[:-13]
...
if __name__ == "__main__":
    if(os.system("gdalbuildvrt>/dev/null 2>&1") == 32512):
        print("gdal not installed on system for macOS use 'brew install gdal'")
        exit(0)
    if(os.system("ncatted>/dev/null 2>&1") == 32512):
        print("NCO not installed on system use if using conda, use command 'conda install nco'")
        exit(0)
    main()